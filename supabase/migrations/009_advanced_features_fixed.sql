...
-- Drop and recreate print_jobs table to ensure correct schema
...
CREATE INDEX idx_print_jobs_printer ON public.print_jobs(printer_name);
CREATE INDEX idx_print_jobs_started ON public.print_jobs(started_at DESC);
CREATE INDEX idx_print_jobs_failed ON public.print_jobs(failed) WHERE failed = true;
...
-- Drop constraint if exists and recreate with correct values
DO $$ 
BEGIN
  IF EXISTS (
...
ALTER TABLE public.leads ADD CONSTRAINT leads_status_check 
  CHECK (status IN ('open', 'contacted', 'qualified', 'converted', 'lost'));
...
DROP POLICY IF EXISTS "Admin can manage print jobs" ON public.print_jobs;
...
DROP POLICY IF EXISTS "Admin can view health snapshots" ON public.business_health_snapshots;
...
DROP POLICY IF EXISTS "Admin can manage reports" ON public.scheduled_reports;
...
DROP POLICY IF EXISTS "Admin can view report history" ON public.report_history;
...
DROP POLICY IF EXISTS "Admin can create activity logs" ON public.admin_activity_logs;
