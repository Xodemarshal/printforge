...
export async function sendEmail(
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'Crafted Tale <noreply@craftedtale.in>',
      to: options.to,
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background-color: #4F46E5; color: white; padding: 20px; text-align: center;">
    <h1 style="margin: 0;">Crafted Tale</h1>
  </div>
  ...
  <div style="text-align: center; padding: 20px; color: #6b7280; font-size: 12px;">
    <p>&copy; ${new Date().getFullYear()} Crafted Tale. All rights reserved.</p>
  </div>
</body>
</html>
  `;
...
export async function createResendDomain(domainName?: string) {
  const name = domainName || process.env.RESEND_DOMAIN || 'craftedtale.in';
