...
export default async function AdminDashboardPage() {
  ...
  const { count: totalRequests } = await supabase.from("stl_uploads").select("*", { count: "exact", head: true });
  const { count: todayRequests } = await supabase.from("stl_uploads").select("*", { count: "exact", head: true }).gte("created_at", `${today}T00:00:00Z`);
  const { count: pendingRequests } = await supabase.from("stl_uploads").select("*", { count: "exact", head: true }).eq("status", "pending_review");
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          ["Idea Requests Today", todayRequests ?? 0],
          ["Total Idea Requests", totalRequests ?? 0],
          ["Pending Reviews", pendingRequests ?? 0]
        ].map(([label, value]) => (
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <a href="/admin/print-queue" className="bg-black border border-gray-700 rounded-lg p-4 text-center text-sm text-gray-300 hover:text-white hover:border-gray-600 transition-colors">
            Review Requests
          </a>
